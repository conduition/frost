...
//!
//! ## Generating XPubs and Deriving Children
//!
...
//!     bip32::{HARDENED_OFFSET, NETWORK_VERSION_XPUB, ExtendedPubkey},
//!     VerifyingKey,
//! };
//! use hex::FromHex;
//!
...
//! assert_eq!(xpub.child_number, HARDENED_OFFSET);
//! assert_eq!(xpub.network_version, NETWORK_VERSION_XPUB);
//! ```
//!
...
//!
//! ```
//! # let xpub = "xpub6DTAAS87qxjh5VzAEr1SdXssEpPBxxKXvGEpTvRxxd9KJMmrsE\
//! #             zqwWs87AHHrnrawQa9GC1xPt3jVHbbHGm9R2m5XpJwT2Red3gYyEA6yVR"
//! #     .parse::<frost_secp256k1_tr::bip32::ExtendedPubkey>()
//! #     .unwrap();
//! use frost_secp256k1_tr::{bip32::ChildIndex, key_path};
//!
//! let first_child = xpub.child(ChildIndex::from(0)).expect("deriving child failed");
//! let deep_child = xpub.derive(key_path!(vk / 0 / 1 / 2 / 8, 4).as_ref()).expect("deriving child failed");
//! ```
//!
...
//!
//! ## Signing with a Child Key
//!
//! When running a FROST signing session, the participants can choose to either sign as the parent
//! [`VerifyingKey`], or to sign as a child key derived from the [`VerifyingKey`] using BIP32 at
//! any valid [`KeyPath`]. To do so, the proposed [`SigningTarget`][crate::SigningTarget]
//! should specify a set of [`SigningParameters`][crate::SigningParameters] whose
//! [`bip32_key_path`][SigningParameters::bip32_key_path] field is populated.
//! This tells the signers which BIP32 child key should be committed to in the signature,
//! and tells the coordinator how to tweak the final aggregated signature so that it
//! will be valid under that child key.
//!
//! ```
//! use std::collections::BTreeMap;
//! use frost_secp256k1_tr::{SigningPackage, SigningParameters, SigningTarget, key_path};
//!
//! let sig_target = SigningTarget::new(
//!     b"message",
//!     SigningParameters {
...
//!
//! let sig_pkg = SigningPackage::new(commitments, sig_target);
//! ```
